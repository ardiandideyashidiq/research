...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--db",
        default="tmp/publications.sqlite",
        help="Path to SQLite database (default: tmp/publications.sqlite)",
    )
    parser.add_argument(
        "--downloads",
        default="data/downloads",
        help="Path to PDF downloads directory (default: data/downloads)",
    )
    parser.add_argument(
        "--log-dir",
        default="logs",
        help="Directory for auto-generated run log files (default: logs)",
    )
    parser.add_argument(
        "--no-log-file",
        action="store_true",
        help="Disable auto-generating log file under logs directory",
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Enable verbose debug logging"
    )
    parser.add_argument(
        "--no-cache", action="store_true", help="Disable persistent HTTP/page caching"
    )
    parser.add_argument(
        "--clear-cache", action="store_true", help="Clear cached HTTP responses"
    )
    ...
    p_pipe.add_argument(
        "--bib", help="Path to seed .bib file or directory with .bib files"
    )
    p_pipe.add_argument(
        "--limit", type=int, default=15, help="Search limit per provider (default: 15)"
    )
    p_pipe.add_argument(
        "--providers",
        help="Comma-separated providers (e.g. arxiv,openalex,crossref,doaj,openaire)",
    )
    p_pipe.add_argument(
        "--no-scholar", action="store_true", help="Exclude Google Scholar"
    )
    p_pipe.add_argument(
        "--no-snowball", action="store_true", help="Skip citation graph snowballing"
    )
    p_pipe.add_argument(
        "--snowball-seeds",
        type=int,
        default=2,
        help="Number of top seed papers to snowball (default: 2)",
    )
    p_pipe.add_argument(
        "--snowball-limit",
        type=int,
    ...
    p_pipe.add_argument(
        "--no-convert", action="store_true", help="Skip converting PDFs to Markdown"
    )
    p_pipe.add_argument(
        "--convert-concurrency",
        type=int,
        default=4,
        help="Concurrent conversion workers (default: 4)",
    )
    p_pipe.add_argument(
        "--snowball-concurrency",
        type=int,
        default=4,
        help="Concurrent snowball workers (default: 4)",
    )
    p_pipe.add_argument(
        "--no-rag", action="store_true", help="Skip semantic chunking and RAG indexing"
    )
    p_pipe.add_argument(
        "--no-streaming",
        action="store_true",
    ...
    p_search = subparsers.add_parser(
        "search",
        help="Search academic literature across federated providers and Google Scholar",
    )
    p_search.add_argument("query", help="Search query string")
    p_search.add_argument(
        "--providers",
        help="Comma-separated providers (arxiv, openalex, crossref, doaj, openaire, scholar)",
    )
    p_search.add_argument(
        "--limit", type=int, default=10, help="Max results to fetch (default: 10)"
    )
    p_search.add_argument(
        "--no-index",
        action="store_true",
        help="Do not save results into SQLite database",
    )

    # 3. snowball
    p_snow = subparsers.add_parser(
        "snowball", help="Expand citation network (citing and referenced papers)"
    )
    ...
    p_dl = subparsers.add_parser(
        "download",
        help="Concurrently download open-access PDFs with %%PDF magic byte verification",
    )
    p_dl.add_argument(
        "--cite-keys",
        help="Comma-separated cite_keys to download (default: all pending)",
    )
    p_dl.add_argument(
        "--concurrency",
        type=int,
    ...
    p_dl.add_argument(
        ...
        type=float,
        default=10.0,
        help="Download timeout in seconds per attempt (default: 10.0)",
    )
    p_dl.add_argument(
        "--force",
        action="store_true",
    ...
    p_conv = subparsers.add_parser(
        "convert", help="Convert PDF(s) to normalized Markdown with layout cleaning"
    )
    ...
    p_conv.add_argument(
        "--index-rag",
        action="store_true",
        help="Also chunk and index into RAG database",
    )
    ...
    p_query.add_argument(
        "--limit", type=int, default=5, help="Number of excerpts to return (default: 5)"
    )
    p_query.add_argument(
        "--cite-key", help="Filter search to a specific paper cite_key"
    )
    p_query.add_argument(
        "--format-context",
        action="store_true",
    ...
    p_exp = subparsers.add_parser(
        "export", help="Export publications database to BibTeX, JSON, or SQLite"
    )
    p_exp.add_argument(
        "--format",
        choices=["bibtex", "json"],
    ...
    p_exp.add_argument(
        "--output", help="Output file path (default: stdout or tmp/export.<ext>)"
    )

    # 8. stats
    subparsers.add_parser(
        "stats",
        help="Show database metrics (publications, downloads, conversions, RAG chunks)",
    )
    ...
    p_putusan.add_argument(
        "path", help="Path to Putusan PDF or directory containing Putusan PDFs"
    )
    p_putusan.add_argument(
        "--output-dir",
    ...
    p_putusan.add_argument(
        ...
        type=int,
        default=None,
        help="Process only N sample PDFs if path is a directory",
    )
    p_putusan.add_argument(
        "--concurrency",
        type=int,
        default=4,
        help="Concurrent workers for batch conversion (default: 4)",
    )
    p_putusan.add_argument(
        "--max-chunk-chars",
        type=int,
    ...
    p_web.add_argument(
        "--no-index",
        action="store_true",
        help="Do not save and index into SQLite RAG database",
    )
    p_web.add_argument(
        "--output-dir",
    ...
    p_cextract.add_argument(
        "--cite-key", help="Extract specific publication by cite_key"
    )
    p_cextract.add_argument(
        "--all",
        action="store_true",
        help="Extract cards for all publications in database",
    )
    p_cextract.add_argument(
        "--corpus",
        choices=["all", "literature", "putusan", "web"],
    ...
    p_cextract.add_argument(
        "--force", action="store_true", help="Re-extract and overwrite existing cards"
    )
    p_cextract.add_argument(
        "--concurrency",
        type=int,
    ...
    p_cextract.add_argument(
        ...
        type=int,
        default=200,
        help="Maximum publications to process (default: 200)",
    )

    # cards list
    p_clist = cards_subs.add_parser(
        "list", help="List literature review cards in database"
    )
    ...
    p_clist.add_argument(
        "--limit", type=int, default=50, help="Max cards to list (default: 50)"
    )

    # cards show
    p_cshow = cards_subs.add_parser(
        "show", help="Show full literature review card details for a paper/putusan"
    )
    p_cshow.add_argument("cite_key", help="Citation key or case identifier")

    # cards edit
    p_cedit = cards_subs.add_parser(
        "edit", help="Update annotations on a literature review card"
    )
    ...
    p_cedit.add_argument(
        "--findings", help="Update key findings / ratio decidendi / verdict"
    )
    ...
    # cards export
    p_cexport = cards_subs.add_parser(
        "export", help="Export literature review matrix table and cards"
    )
    ...
    p_cexport.add_argument(
        "--output", help="Output file path (default: data/literature_matrix.<ext>)"
    )
    p_cexport.add_argument(
        "--corpus",
        choices=["all", "literature", "putusan", "web"],
    ...
    p_bib = subparsers.add_parser(
        "bib", help="Full CRUD bibliography manager with multi-CSL citation styling"
    )
    bib_subs = p_bib.add_subparsers(dest="bib_action", help="Bibliography action")

    # bib list
    p_blist = bib_subs.add_parser(
        "list", help="List and format bibliography entries in CSL style"
    )
    ...
    p_blist.add_argument(
        "--limit", type=int, default=20, help="Maximum entries to list (default: 20)"
    )
    p_blist.add_argument(
    ...
    # bib show
    p_bshow = bib_subs.add_parser(
        "show", help="Show reference formatted in one or all CSL styles"
    )
    ...
    p_bshow.add_argument(
        "--in-text", action="store_true", help="Also display in-text citation format"
    )

    # bib add
    p_badd = bib_subs.add_parser(
        "add", help="Manually add a new reference to the bibliography"
    )
    p_badd.add_argument("--title", required=True, help="Publication title")
    p_badd.add_argument(
        "--author",
        action="append",
        help="Author name (can be repeated or comma-separated)",
    )
    ...
    p_badd.add_argument(
        "--entry-type",
        default="article",
        help="Entry type: article, book, inproceedings, putusan, etc. (default: article)",
    )
    p_badd.add_argument(
        "--cite-key", help="Custom citation key (auto-generated if omitted)"
    )

    # bib update
    p_bupdate = bib_subs.add_parser(
        "update", help="Update fields of an existing bibliography entry"
    )
    ...
    # bib delete
    p_bdel = bib_subs.add_parser(
        "delete", help="Delete a reference from the bibliography"
    )
    ...
    # bib import
    p_bimp = bib_subs.add_parser(
        "import", help="Import references from BibTeX, CSL-JSON, RIS, or DOI"
    )
    ...
    # bib export
    p_bexp = bib_subs.add_parser(
        "export", help="Export bibliography formatted in selected CSL style and format"
    )
    ...
    p_bexp.add_argument(
        ...
        type=int,
        default=500,
        help="Maximum entries to export (default: 500)",
    )

    # 14. workflow (20-Stage Normative Legal Research Lifecycle)
    p_wf = subparsers.add_parser(
        "workflow",
        help="Track normative legal research workflow (20 stages & 7 phases)",
    )
    wf_subs = p_wf.add_subparsers(dest="workflow_action", help="Workflow action")

    # workflow init
    p_wf_init = wf_subs.add_parser(
        "init", help="Initialize or reset normative research project"
    )
    p_wf_init.add_argument("title", help="Title of thesis or research project")
    p_wf_init.add_argument("--author", default="", help="Author / researcher name")
    p_wf_init.add_argument(
        "--typology",
        choices=["vague_norm", "wet_vacuum", "conflict_van_normen"],
        default="",
        help="Norm problem typology (Tahap 2)",
    )
    p_wf_init.add_argument(
        "--approaches",
        help="Comma-separated legal approaches (e.g. statute,conceptual,case)",
    )
    p_wf_init.add_argument(
        "--questions", help="Semicolon-separated research questions (Tahap 4)"
    )

    # workflow status
    wf_subs.add_parser(
        "status", help="Show visual progress across 7 phases and 20 stages"
    )

    # workflow check
    p_wf_check = wf_subs.add_parser(
        "check", help="Record gate-check evaluation for a stage (1-20)"
    )
    p_wf_check.add_argument("stage", type=int, help="Stage number (1 to 20)")
    p_wf_check.add_argument(
        "--pass",
        dest="passed",
        action="store_true",
        default=True,
        help="Mark stage gate check as passed",
    )
    p_wf_check.add_argument(
        "--fail",
        dest="passed",
        action="store_false",
        help="Mark stage gate check as not passed",
    )
    p_wf_check.add_argument(
        "--notes", help="Researcher notes or operational evidence for this stage"
    )
    p_wf_check.add_argument(
        "--typology",
        choices=["vague_norm", "wet_vacuum", "conflict_van_normen"],
        help="Update norm typology (Tahap 2)",
    )
    p_wf_check.add_argument(
        "--approaches", help="Update comma-separated approaches (Tahap 3)"
    )
    p_wf_check.add_argument(
        "--questions", help="Update semicolon-separated research questions (Tahap 4)"
    )
    p_wf_check.add_argument("--grand", help="Update Grand Theory (Tahap 6)")
    p_wf_check.add_argument("--middle", help="Update Middle Range Theory (Tahap 6)")
    p_wf_check.add_argument("--applied", help="Update Applied Theory (Tahap 6)")
    p_wf_check.add_argument(
        "--principles", help="Update comma-separated legal principles (Tahap 7)"
    )

    # workflow export
    p_wf_exp = wf_subs.add_parser(
        "export", help="Export complete methodology audit trail markdown document"
    )
    p_wf_exp.add_argument(
        "--output",
        default="logs/metodologi_audit.md",
        help="Destination markdown file path",
    )

    # 15. irac (Deductive Syllogism & Logic Reasoning)
    p_irac = subparsers.add_parser(
        "irac",
        help="Formal deductive legal syllogism (IRAC: Issue, Rule, Analysis, Conclusion)",
    )
    irac_subs = p_irac.add_subparsers(dest="irac_action", help="IRAC action")

    # irac add
    p_irac_add = irac_subs.add_parser(
        "add", help="Add new IRAC deductive legal syllogism"
    )
    p_irac_add.add_argument(
        "--issue", required=True, help="Issue / legal problem statement (I)"
    )
    p_irac_add.add_argument(
        "--rule",
        required=True,
        help="Rule / Premis Mayor / abstract statutory norm (R)",
    )
    p_irac_add.add_argument(
        "--facts",
        required=True,
        help="Facts / Premis Minor / concrete facts & subsumption (A)",
    )
    p_irac_add.add_argument(
        "--conclusion",
        required=True,
        help="Conclusion / prescriptive legal finding (C)",
    )
    p_irac_add.add_argument(
        "--question-idx",
        type=int,
        default=1,
        help="Associated research question index (default: 1)",
    )
    p_irac_add.add_argument(
        "--domain",
        choices=["pidana", "perdata", "tata_negara", "administrasi", "umum"],
        default="umum",
        help="Legal domain branch (default: umum)",
    )
    p_irac_add.add_argument(
        "--method",
        default="interpretasi_gramatikal",
        help="Method of interpretation/construction (e.g. interpretasi_teleologis, analogi, a_contrario)",
    )
    p_irac_add.add_argument(
        "--cite-keys", help="Comma-separated citation keys linked to this syllogism"
    )

    # irac list
    p_irac_list = irac_subs.add_parser(
        "list", help="List all deductive legal syllogisms"
    )
    p_irac_list.add_argument(
        "--question-idx", type=int, help="Filter by research question index"
    )

    # irac validate
    irac_subs.add_parser(
        "validate",
        help="Validate syllogisms for legal logic fallacies and analogy guard",
    )

    # irac export
    p_irac_exp = irac_subs.add_parser(
        "export", help="Export IRAC blocks to Markdown for thesis chapters"
    )
    p_irac_exp.add_argument("--output", help="Output file path (default: stdout)")
    p_irac_exp.add_argument(
        "--question-idx", type=int, help="Filter by research question index"
    )

    # 16. scaffold (5-Chapter Thesis Draft Generator)
    p_scaffold = subparsers.add_parser(
        "scaffold",
        help="Generate structured 5-chapter skripsi markdown drafts linked to research state",
    )
    p_scaffold.add_argument(
        "--output-dir",
        default="draft_skripsi",
        help="Destination folder for thesis markdown files (default: draft_skripsi)",
    )
    p_scaffold.add_argument(
        "--style",
        choices=["indonesia", "apa", "ieee", "oscola", "chicago"],
        default="indonesia",
        help="Citation style for DAFTAR_PUSTAKA.md (default: indonesia)",
    )
    p_scaffold.add_argument(
    ...
    p_audit = subparsers.add_parser(
        "audit-traceability",
        help="Audit thesis drafts for citation traceability, ghost citations, and primary sources",
    )
    p_audit.add_argument(
        "--draft-dir",
        default="draft_skripsi",
        help="Path to thesis markdown directory (default: draft_skripsi)",
    )
    p_audit.add_argument("--output", help="Path to write audit report markdown file")
