...
class ResearchPipeline:
    ...
    async def run(
        ...
        **kwargs: Any,
        ...
        t0 = time.time()
        logger.info("Starting Research Pipeline for query: '{}'", cfg.query)

        # 1. Federated Literature Discovery
        discovered: list[Any] = []
        try:
        ...
        # 2. Citation Graph Snowballing
        snowballed_count = 0
        if cfg.snowball and discovered:
            seeds = [r for r in discovered if r.doi or r.title][:cfg.snowball_seeds]
            for seed in seeds:
                try:
                    ...
                    snowballed_count += len(res.forward_citing) + len(res.backward_references)
                except Exception as exc:  # noqa: BLE001
        ...
        # 3. Open-Access Paper Downloading
        downloaded_count = 0
        if cfg.download:
        ...
        # 4. PDF to Markdown Conversion & Layout Normalization
        converted_count = 0
        indexed_chunks_count = 0

        if cfg.convert:
            downloaded_pubs = self.app.db.list(status="downloaded", limit=100)
            for pub in downloaded_pubs:
                ...
                try:
                    ...
                    # 5. Semantic Chunking & RAG FTS5 Indexing
                    if cfg.index_rag:
                        chunks = self.app.retriever.index_document(conv_doc, cite_key=pub.cite_key)
